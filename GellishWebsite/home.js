...
document.addEventListener('DOMContentLoaded', function() {
     ...
     setTimeout(() => {
       ...
       window.scrollTo(0,0); //scrolls to the top of the page once pop up appears

       lineWatcher.style.display = 'none';

       //prevents scrolling down
       window.addEventListener('scroll',preventScroll);

       //changes the display from not being visible to visible
 ...
 function removeShape() {
  popup.style.display = 'none'; // Remove the "pop-up" class, effectively hiding the pop-up
  background.style.display = 'none'; //removes the darker background
  lineWatcher.style.display = 'block';
  // Optionally, you can also hide the pop-up by changing its style to `display: none`
  document.body.classList.remove('no-scroll'); // This ensures the pop-up is hidden visually
  window.removeEventListener('scroll',preventScroll);
}



//changes the  color from peach to dark brown to preach when you scroll down or up.
...
//creats the scroll effect for the reviews
window.addEventListener('scroll', function() {
  ...
  let translateX = -scrollPos * 0.5; // Change 0.5 to increase/decrease speed
  let translateOtherX = +scrollOther * 0.5;
